...
export function ProjectSectionV4({
 ...
 return (
  <section className={cn("section-padding-primary overflow-hidden", backgroundColor.light, `dark:${backgroundColor.dark}`)}>
     ...
     >
      ...
      {keyPoints && keyPoints.length > 0 && (
       <div className={cn(
        ...
        keyPointsLayout === 'grid' ? "grid sm:grid-cols-2" : "flex flex-col"
       )}>
        {keyPoints.map((keyPoint, index) => (
         <div
          ...
          className={cn(
           "group",
           isCard
            ? "flex items-center gap-5 min-h-[64px] rounded-5 border border-primary px-6 py-2 md:min-h-[85px] md:px-[1.875rem] md:py-3"
            : keyPointsLayout === 'grid' ? "flex flex-col text-left" : "flex items-start gap-5"
          )}
         >
          <span className={cn(
           "text-lg text-primary",
           isCard ? "duration flex-none scale-100 transition-transform ease-linear group-hover:scale-90" : "flex-none"
          )}>
