...
const megaMenuData = {
	Home: {
		rightSidebar: [
			{ name: "Get Started", icon: <BookOpen className="h-4 w-4" /> },
			{ name: "Setup 101", icon: <Settings className="h-4 w-4" /> },
			{ name: "Adding Users", icon: <UserPlus className="h-4 w-4" /> },
			{ name: "Libraries and SDK", icon: <Library className="h-4 w-4" /> },
			{ name: "Adding Projects", icon: <FolderOpen className="h-4 w-4" /> },
			{ name: "Mails", icon: <Mail className="h-4 w-4" /> },
		],
		content: {
			"Get Started": [
				{
	...
	Solutions: {
		rightSidebar: [
			{ name: "Web Solutions", icon: <Globe className="h-4 w-4" /> },
			{ name: "Mobile Apps", icon: <Smartphone className="h-4 w-4" /> },
	...
	Services: {
		rightSidebar: [
			...
			{ name: "Design", icon: <Camera className="h-4 w-4" /> },
	...
	Companies: {
		rightSidebar: [
			{ name: "Startups", icon: <Briefcase className="h-4 w-4" /> },
			{ name: "Enterprise", icon: <Database className="h-4 w-4" /> },
		],
		content: {
			Startups: [
				{
	...
	Portfolio: {
		rightSidebar: [
			{ name: "Recent Work", icon: <FolderOpen className="h-4 w-4" /> },
			{ name: "Case Studies", icon: <FileText className="h-4 w-4" /> },
		],
		content: {
			"Recent Work": [
				{
...
export function Header() {
	...
	return (
		<header className="sticky top-0 z-50 w-full border-b border-gray-200 bg-white/95 shadow-sm backdrop-blur-sm">
			<div className="px-4 sm:px-6 lg:px-8">
				<div className="flex h-16 items-center">
					...
					<nav className="hidden space-x-8  md:flex">
						{Object.keys(megaMenuData).map((menuItem) => (
							...
							>
								<a
									href="#"
									className="px-1 lg:px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:text-gray-900"
								>
									{menuItem}
								</a>
							</div>
						...
						<Link
							href="/about"
							className="px-1 lg:px-3 pt-1 text-sm font-medium text-gray-700 transition-colors hover:text-gray-900"
						>
							AboutUs
						</Link>
					</nav>
					...
					<div className="flex items-center gap-2 pl-10">
						<button
							type="button"
							className="inline-flex items-center justify-center rounded-md p-2 text-gray-600 hover:bg-gray-100 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-600 md:hidden"
						...
						<div className="hidden flex-col gap-3 md:flex">
							<ContactUsDialog>
								<Button
									className={cn("rounded-full px-2 py-1 text-sm capitalize")}
									size="default"
								>
			...
			{mobileOpen && (
				<div className="absolute left-0 right-0 z-50 w-full border-t border-gray-200 bg-white shadow-xl md:hidden">
						...
						<Accordion type="single" collapsible className="w-full">
							{Object.keys(megaMenuData).map((menuItem) => {
								...
								if (typeof menuData === "string") {
									return (
										<div key={menuItem} className="py-2">
											<Link
												href={menuData}
						...
						<div className="overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/5">
							<div className="px-6 py-8 sm:px-8 lg:px-10">
								<div className="flex">
																	...
																	{section.items.map(
																		(item: any, itemIndex: number) => (
																			<a
																				key={itemIndex}
																				href="#"
																				className="group flex items-start space-x-3 rounded-lg p-2 transition-colors hover:bg-gray-50"
																			>
																				<div className="mt-1 flex-shrink-0 text-gray-400 transition-colors group-hover:text-blue-600">
																					{item.icon}
																				</div>
									...
									{/* Right Sidebar */}
									<div className="w-64 shrink-0 border-l border-gray-200 pl-8">
										<div className="space-y-2">
											{(
												megaMenuData[
													activeMenu as keyof typeof megaMenuData
												] as any
											)?.rightSidebar?.map((item: any, index: number) => (
												<a
													key={index}
													href="#"
													className={`group flex items-center space-x-3 rounded-lg p-3 transition-colors ${
