  ...
  <Container>
   <section className={cn('mx-auto max-w-6xl py-12 md:py-16', className)}>
    ...
    <div className="relative mb-8 pb-1">
     <div className="relative z-10 grid grid-cols-2 gap-3 divide-x divide-gray-300 md:flex md:divide-x-0 md:space-x-16 md:justify-center">
      {tabs.map((tab, idx) => {
       const isLastOdd = tabs.length % 2 === 1 && idx === tabs.length - 1;
       return (
        <button
         key={tab.id}
         onClick={() => setActiveTab(tab.id)}
         type="button"
         className={`w-full md:w-auto inline-block transform rounded-none md:rounded-t-full px-4 py-2 text-sm font-medium transition-colors duration-200 md:text-base
              ${activeTab === tab.id
     ...
     <div className="absolute bottom-0 left-0 right-0 z-20 h-[2px] translate-y-[-4px] bg-gray-200 dark:bg-gray-700 hidden md:block" />
    </div>
