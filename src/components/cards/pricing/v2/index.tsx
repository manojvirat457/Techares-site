...
export function PricingCard({
  ...
  return (
    <div className="group rounded-5 bg-white p-10 pb-[50px] text-center shadow-3 dark:bg-accent-700 dark:shadow-none">
      ...
      <div
        className={cn(
          'relative z-1 mx-auto mb-30px mt-5 flex min-h-[80px] max-w-[274px] items-center justify-center rounded-5 bg-accent-300 font-secondary text-xl font-bold leading-[1.25] text-accent-900 transition-colors duration-300 dark:bg-accent-900 dark:text-white lg:text-2xl',
      ...
      {features && features.length > 0 && (
        ...
        >
          {features.map((feature, index) => (
            <li
              className="flex items-center gap-3 font-secondary text-md font-bold leading-[1] text-accent-800 dark:text-white lg:gap-4"
              key={index}
            >
      ...
      <div className="flex-none">
        <Button
          ...
          className={cn(
            'flex w-full',
            'border-2 border-primary bg-transparent text-accent-900',
            'after:bg-primary'
          )}
        >
